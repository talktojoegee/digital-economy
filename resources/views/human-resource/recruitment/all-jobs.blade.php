                ...
                <div class="card-body">
                    <div class="media align-items-center">
                        ...
                        <div class="media-body text-right">
                            <p class="fs-18 text-white mb-2">Open</p>
                            <span class="fs-48 text-white font-w600">{{number_format($jobs->where('status',1)->count())}}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card bg-info">
                <div class="card-body">
                    <div class="media align-items-center">
                        ...
                        <div class="media-body text-right">
                            <p class="fs-18 text-white mb-2">Closed</p>
                            <span class="fs-48 text-white font-w600">{{number_format($jobs->where('status',2)->count())}}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="row mb-3">
                        <div class="col-md-12">
                            ...
                            @endif
                            <a href="{{route('post-job')}}" class="btn btn-sm btn-primary float-right"> <i class="ti-plus mr-2"></i> Post New Job</a>
                        </div>
                    </div>
                        ...
                        <table id="example3" class="display table-responsive-lg">
                            ...
                            @foreach($jobs as $job)
                                <tr>
                                    <td>{{$serial++}}</td>
                                    <td>
                                        <a href="{{route('view-job', $job->slug)}}" class="text-primary">{{strlen($job->job_title) > 31 ? substr($job->job_title,0,31).'...' : $job->job_title  }}</a>
                                    </td>
                                    <td>{{$job->getDepartment->department_name ?? '' }}</td>
                                    <td>{{$job->getJobRole->role_name ?? '' }}</td>
                                    <td class="text-danger">{{date('d M, Y', strtotime($job->deadline))  }}</td>
                                    <td>
                                        @switch($job->status)
                                            @case(1)
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <a href="{{route('edit-job', $job->slug)}}" class="btn btn-warning text-white shadow btn-xs sharp mr-1"><i class="ti-pencil"></i></a>
                                            <a href="{{route('view-job', $job->slug)}}" class="btn btn-primary shadow btn-xs sharp mr-1"><i class="ti-eye"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
